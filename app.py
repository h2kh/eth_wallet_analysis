...
with colb1:
    ...
    if wallet_validator(wallet_address):
        ...
        fig.update_layout(title={'text':'<b>Largest counterparty wallets<b>', 'font': {'size': 20}, 'xanchor': 'center', 'x':0.5})
        ...
        fig.update_yaxes(
                tickfont = {"size": 15},
                title_font={'size':15, 'family':'verdana', 'color':'indigo'}
                )
        ...
        fig2.update_layout(title={'text': '<b>ETH transfers over the selected time period<b>', 'font': {'size': 20}, 'xanchor': 'center', 'x':0.5})
        ...
        if len(sunburst_df) > 0:
            ...
            fig3.update_layout(title={'text': '<b>ETH flows<b>', 'font': {'size': 20}, 'xanchor': 'center', 'x':0.5})
...
The different API endpoints used, in addition to my mental model of an ETH wallet transaction, influenced my design of the database. The unified view of the transactions data displayed above comes from joining and filtering the different tables in the database.\n""")
    ...
    st.write("""This database is always checked first whenever a new query is submitted by the app user in order to avoid unnecessary requests to the API endpoints.\n
If the desired data isn't available in the database, then the API endpoints are used. Since the ETL process is triggered by the user submitting a query and is not run on a schedule, using an orchestrator like Airflow isn't appropriate.\n

To do:
- Figure out how to scrape (without getting IP-blocked) wallet name tags from EtherScan since they aren't provided by the API.
